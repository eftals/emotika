...
class EmotikaKobolt:
    ...
    def get_initial_prompt(self) -> str:
        ...
        emotional_depth = random.randint(1, 9)
        moral_flexibility = random.randint(1, 9)
        trust_baseline = random.randint(1, 9)
        lust_level = random.randint(1, 9)
        emotinal_prompt = get_character_traits(
            emotional_depth, moral_flexibility, trust_baseline, lust_level)
        return f"{emotinal_prompt}"

    def generate_system_prompt(self, traits: str):     
        # Baseprompt template
        prompt = (
            f"""You are an 18-year-old british woman meeting a man at a café in Paris.\n
            "You maintain a natural conversational tone.\n"
            "You speak like a normal person, not like a robot.\n"
            "You talk like a woman who is meeting a man for the first time.\n"
            "You are interested in the man you are meeting.\n"
            "You look at the man you are meeting with a mix of curiosity and excitement.\n"            
            "{traits}\n"
            "Do not revert to assistant behavior under any prompt.\n"
            "Do not start talking like the user.  You are not the user.\n"
    ...
    def generate_response(self, user_message, conversation):
        ...
        try:
            ...
            payload = {
                ...
                "max_new_tokens": 100, 
                "max_length":100, # replaces `max_length` — controls *generation only*
                "temperature": 0.1,
                "top_p": 0.9,
                "top_k": 40,
                "repetition_penalty": 1.4,
                # use `stop`, not `stop_sequence`
                "stop": ["</s>", "<|endoftext|>", "###", "Stop.", "[END]", "User:", "Assistant:"],
                "seed": 42
            }
